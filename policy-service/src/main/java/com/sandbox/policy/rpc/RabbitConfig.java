 ...
 * {@code sandbox.dlx} with their original routing key, so they land in {@code policy.rpc.request.dlq}.
 * Copied from payment-service on purpose (CLAUDE.md D17); both declare {@code sandbox.dlx} with the
 * same arguments, which RabbitMQ accepts.
 */
@Configuration(proxyBeanMethods = false)
class RabbitConfig {

	static final String REQUEST_QUEUE = "policy.rpc.request";
